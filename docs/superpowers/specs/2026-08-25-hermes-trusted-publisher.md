...
7. All PR history for the exact branch is enumerated. No second PR may be created. One closed, unmerged, exact-repository PR is reopened and reused; a merged or ambiguous prior PR is rejected. The open readback must bind the exact feature head, current target-base SHA, `develop`, owner, and non-fork repository identity.
