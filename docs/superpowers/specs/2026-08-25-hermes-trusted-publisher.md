...
The publisher job is installed disabled-first. Its installer migrates only the exact prior deployed v1 backup allowlist, snapshots every newly managed publisher dependency before any installer mutation, and writes one authenticated complete v2 manifest; any partial, enlarged, duplicate, or injected set fails with zero writes. The job may be enabled only when the installed Hermes runtime exposes both `hermes.trusted_local_commit.v1` and `hermes.worker_git_isolation.v1`; produces and verifies a host-signed authority receipt binding repository, board, project, task, run, workspace, branch, base, head, and changed paths; passes a semantic claim/completion CAS canary; passes the exact model-path denial/success canary; passes the host credential probe; and passes all repository activation tests. The denial receipt must explicitly cover profile `.env`, GitHub config/token, SSH config/private keys, Keychain lookup, loopback/public networking, and Git metadata, while also proving an ordinary workspace edit and bounded test succeed. Until the reviewed runtime supplies every exact field and behavior, activation remains `PENDING_HERMES_RUNTIME`. The Mac mini is not changed by this implementation PR; installation occurs only after both the Hermes broker PR and this publisher PR independently pass review and merge.
