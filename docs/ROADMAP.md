...
- **Guideline currency**: standing review of guideline revisions against what each calculator implements. The [guideline/source registry](../ops/hermes/radulator/skills/radulator-operations/references/guideline-versions.json) is the authoritative per-calculator record; a verified row is limited to its cited claims and executable evidence, while other rows remain seed-unverified.
  - **Implemented and source-verified**: Bosniak version 2019 CT classification and MELD 3.0 (OPTN Policy 9.1.D). These are bounded claims; every other row retains its separate registry status.
    ...
    - **Temporary BI-RADS rollback evidence**: the live replacement is clearly labeled `Legacy ACR BI-RADS Fifth Edition (2013) temporary rollback` while the sixth-edition manual is pending. The [official ACR BI-RADS system page](https://www.acr.org/Clinical-Resources/Clinical-Tools-and-Reference/Reporting-and-Data-Systems/BI-RADS) supplies the three-modality scope. The historically named `npm run test:birads-fda-source` now retrieves the digest-pinned official [ACR BI-RADS Atlas Fifth Edition Quick Reference](https://edge.sitecorecloud.io/americancoldf5f-acrorgf92a-productioncb02-3650/media/ACR/Files/RADS/BI-RADS/BIRADS-Poster.pdf). Claims `acr-birads-three-modality-scope`, `acr-fifth-edition-descriptor-groups`, and `acr-fifth-edition-assessment-labels` cover only the poster's mammography/ultrasound/MRI scope, descriptor groups, Category 0 modality wording, and assessment labels 0-6. Full likelihood, management, and descriptor-combination validation remains gated on the manual.
