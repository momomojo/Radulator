...
- **Guideline currency**: standing review of guideline revisions against what each calculator implements. The [guideline/source registry](../ops/hermes/radulator/skills/radulator-operations/references/guideline-versions.json) is the authoritative per-calculator record; a verified row is limited to its cited claims and executable evidence, while other rows remain seed-unverified.
  - **Implemented and source-verified**: Bosniak version 2019 CT classification; MELD 3.0 (OPTN Policy 9.1.D); and the FDA 2023 MQSA Final Rule (enforced September 10, 2024) with ACR BI-RADS v2025 context for the mammography assessment guide. These are three bounded claims; every other row retains its separate registry status.
    - **Bosniak v2019 CT evidence**: the open [Silverman et al. primary publication](https://pmc.ncbi.nlm.nih.gov/articles/PMC6677285/) is retrieved by `npm run test:bosniak-source`; source-derived vectors include `exactly-70-hu-homogeneous-noncontrast-mass-category-ii` and `exactly-4-mm-obtuse-margin-enhancing-nodule-category-iv`.
    - **MELD 3.0 evidence**: the official [OPTN Policy 9.1.D](https://www.hrsa.gov/sites/default/files/hrsa/optn/optn_policies.pdf) equation and [MELD/PELD calculator user guide](https://www.hrsa.gov/sites/default/files/hrsa/optn/meld-peld-calculator-user-guide.pdf) entry domains are checked by `npm run test:hermes-guideline-registry`; bound cases include `meld3-adult-female-sex-term`, `meld3-dialysis-creatinine-cap`, and the literal endpoint/outside-domain vectors.
    - **Mammography guide evidence**: the FDA source is the [2023 MQSA Final Rule](https://www.fda.gov/radiation-emitting-products/mammography-quality-standards-act-mqsa-and-mqsa-program/important-information-final-rule-amend-mammography-quality-standards-act-mqsa), with enforcement beginning September 10, 2024—not a rule named “MQSA 2024.” `npm run test:birads-fda-source` retrieves the official FDA pages and binds the communication, referral, prior-comparison, and marker-placement requirements to executable cases including `category-4`, `incomplete-prior-comparison`, and `post-procedure-marker`. The ACR v2025 citation supplies version context; this entry does not claim full implementation of every 6th-edition mammography, ultrasound, and MRI descriptor or workflow.
