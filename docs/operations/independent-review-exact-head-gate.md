...
The trusted classifier, not the judge, sets the minimum tier. Calculator runtime changes, shared clinical data/state/bootstrap paths, and trusted release-control or CI-selection paths are high risk. Clinical documentation patches are high risk when they change numeric values, formulas, thresholds, units, scoring, contraindications, interpretations, management, follow-up, staging, or guideline versions. Missing or truncated clinical patch data, clinical paths present only as `previous_filename`, and an explicit `Risk-Tier: high` PR declaration are also high risk. PR evidence and full GitHub file metadata are bound into the exact risk record. A PR may not combine clinical runtime and release-control changes because that would let the candidate redefine its own evidence; those changes must be released sequentially. Feedback-only and other changes are standard risk and still require the primary judge.

## CI binding

Standard-risk PRs targeting `develop` require `Smoke Tests` and `Targeted Calculator Tests`. High-risk `develop` PRs and every PR targeting `main` additionally require `Full Test Suite`. Authors schedule Full on `develop` with the canonical `<!-- radulator-risk: high -->` PR-body marker; opening, synchronizing, reopening, or editing the PR starts a fresh E2E run. The trusted gate independently classifies the actual changed files, so omitting the marker cannot downgrade risk: the missing Full evidence fails closed. Evidence is accepted only from the configured E2E workflow id and GitHub Actions App id, on one exact successful workflow run associated with the current PR head/base. Duplicate same-name checks are ambiguous and fail closed.
