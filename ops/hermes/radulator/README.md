...
The trusted publisher is a separate `no_agent` host process. AI workers edit and test only; the dispatcher-sealed Hermes broker creates a credential-free local commit and parks the task with a `hermes.trusted_local_commit.v1` event. Before any publication, the publisher acquires a durable runtime claim bound to the exact board, project, task, run, worktree, branch, parent, head, and changed paths. It independently rejects repository-local Git network, TLS, proxy, credential-helper, transport-helper, include, protocol, and URL-rewrite configuration, then revalidates local authority immediately before its fixed-binary credentialed push. It uses a plain non-force push, enumerates the complete same-branch PR history across every base and state, opens or reuses exactly one `develop` PR, reads back the exact remote head and PR, and waits for the newest exact E2E run/attempt/check suite from the expected GitHub Actions App before adding `ready-for-gate`. It then creates the separate release tracker and closes only the implementation task through the runtime's atomic task/run/branch/tracker authority CAS. Retries reuse exact remote/PR/tracker/claim state; any differing remote ref, ambiguous or prior PR, stale event, concurrent lifecycle change, or missing readback fails closed. GitHub credentials are loaded only by the no-agent wrapper after the worker turn ends and never enter the worker or broker.
...
- The installed Hermes runtime must expose both reviewed constants, `hermes.trusted_local_commit.v1` and `hermes.worker_git_isolation.v1`, plus the versioned publisher authority claim/completion-CAS API. Activation must also execute `run_worker_model_path_denial_canary()` and receive the exact `hermes.worker_model_path_denial_canary.v1` denial receipt. Until those runtime capabilities are installed, enablement remains disabled and returns `PENDING_HERMES_RUNTIME`.
...
Activation requires `cron.max_parallel_jobs: 1` in both judge profiles, the exact installed `hermes.trusted_local_commit.v1` broker contract, the exact installed `hermes.worker_git_isolation.v1` worker-security boundary, and a working host `gh auth token --hostname github.com` readback. It refuses missing/mismatched keys, a local/GitHub public-map mismatch, or any failed release, judge, publisher, deployment, lifecycle, invariant, lint, or build self-test. The publisher wrapper never sources the Hermes `.env`: it discards inherited GitHub-token/config overrides, resolves the token from the fixed host GitHub CLI without printing it, validates it, and exports it only into the no-agent publisher process. If either installed publisher asset has drifted while its job is enabled, the installer first pauses and reads back the job, then replaces the assets. Plain `--apply` leaves that upgraded publisher disabled; only explicit `--apply --enable` reenables it after exact installed-asset, broker, worker-boundary, GitHub-authentication, and repository-test preflight. A failed post-copy preflight restores the prior publisher bytes and leaves the job paused. Activation then pauses legacy `pr-gate-poller` and `judge-queue` jobs in either judge profile with the reason `replaced-by-radulator-signed-clinical-gate`. Disabled-first installation leaves them unchanged, so there is no unguarded interval before the signed replacement is ready. The baseline restore returns both profiles' complete cron files—including those legacy jobs—to their exact pre-install bytes.
