...
The trusted publisher is a separate `no_agent` host process. AI workers edit and test only; the dispatcher-sealed Hermes broker creates a credential-free local commit and parks the task with a `hermes.trusted_local_commit.v1` event. The publisher independently revalidates the exact board, project, task, worktree, branch, parent, head, changed paths, Git config, and origin. It uses a plain non-force push, opens or reuses exactly one `develop` PR, reads back the exact remote head and PR, and waits for exact-head Smoke Tests, Targeted Calculator Tests, and Hermes Release Control Tests from the expected GitHub Actions App before adding `ready-for-gate`. It then creates the separate release tracker and closes only the implementation task. Retries reuse exact remote/PR/tracker state; any differing remote ref, ambiguous PR, stale event, or missing readback fails closed. GitHub credentials are loaded only by the no-agent wrapper after the worker turn ends and never enter the worker or broker.
...
- The installed Hermes runtime must expose the reviewed `hermes.trusted_local_commit.v1` broker contract. The installer refuses to enable the trusted publisher before that exact runtime import succeeds.
