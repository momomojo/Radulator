...
Current reconciliation authority: authenticated Formspree repair rotates over the privacy-minimized durable receipt set rather than current Gmail search summaries, so receipts beyond Gmail's newest-100 window remain recoverable without persisting raw Gmail ids. Lifecycle apply acquires one exclusive ledger lease for the complete external action plan, verifies every exact frozen event under that lease, and holds it through every action; a prior append rejects the plan before mutation, while a later append waits for the plan to finish.
