...
Missing or contradictory ledger state is repaired only from an explicit operator-reviewed JSON spec. `lifecycle_controller.py reconcile` is read-only unless `--apply` is supplied, preflights every named tracker and source task/receipt through Kanban before any mutation, rejects partial PR/head/base tuples and mismatched receipt digests, bootstraps missing trackers only to `feedback`, and never infers approval, deployment, smoke, learning, or completion. Omit `pr`, `head_sha`, and `base_sha` together when authoritative mapping is unavailable; when supplied, all three must be present in Kanban readback. A terminal Kanban tracker with a nonterminal ledger remains nonterminal and receives an idempotent runnable corrective prerequisite. Example protected state file:
