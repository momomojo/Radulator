...
- The installed Hermes runtime must expose both reviewed constants, `hermes.trusted_local_commit.v1` and `hermes.worker_git_isolation.v1`, plus host-signed full-identity publisher authority claim/verification/completion-CAS APIs. Activation executes a semantic CAS canary that proves exact identity binding, conflict/stale-run/stale-tracker rejection, atomic completion, idempotent replay, and receipt-signature verification. It must also execute `run_worker_model_path_denial_canary()` and receive the exact `hermes.worker_model_path_denial_canary.v1` receipt proving denial of profile `.env`, GitHub config/token, SSH config/private keys, Keychain lookup, loopback network, public network, and Git-metadata writes while proving an ordinary workspace edit and bounded test succeed. Any missing or extra receipt field fails closed. Until those runtime capabilities are installed, enablement remains disabled and returns `PENDING_HERMES_RUNTIME`.
