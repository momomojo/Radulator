...
Missing or contradictory ledger state is repaired only from an explicit operator-reviewed JSON spec. `lifecycle_controller.py reconcile` is read-only unless `--apply` is supplied, freezes and validates the complete Kanban/ledger/action plan before its first mutation, rejects partial PR/head/base tuples and mismatched receipt digests, bootstraps missing trackers only to `feedback`, and never infers approval, deployment, smoke, learning, or completion. PR authority uses a delimited exact token or structured task field; head/base/digest evidence and prerequisite body/assignee checks are scoped to the exact current task rather than nested event history. Omit `pr`, `head_sha`, and `base_sha` together when authoritative mapping is unavailable; when supplied, all three must be present in the exact tracker readback. A terminal Kanban tracker with a nonterminal ledger remains nonterminal and receives an idempotent runnable corrective prerequisite. An open tracker blocked directly after `NEEDS_FIX` recovers actions only from that immediately preceding exact event, so a stale verdict cannot select or reverse a newer correction edge. Example protected state file:
