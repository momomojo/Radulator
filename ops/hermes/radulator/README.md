...
Current reconciliation authority: authenticated Formspree repair rotates over the privacy-minimized durable receipt set, while new intake advances through bounded Gmail API pages with only the opaque page token in authenticated state; repeated runs therefore fetch message 101 and later without persisting raw Gmail ids. Lifecycle apply acquires one exclusive ledger lease for the complete external action plan and, under that lease, compares exact tracker/source Kanban fields, comments, and relations immediately before and after every external action; stale preflight authority is rejected before the first mutation, only the exact expected action delta is accepted afterward, a prior ledger append rejects the plan, and a later append waits for the plan to finish.
...
The feedback intake uses the Radulator Formspree sender/subject only as a bounded Gmail search prefilter. Before it creates any durable task, it requires Gmail's topmost `Authentication-Results` field to show aligned Formspree DKIM, SPF, and DMARC passes; a visible-header lookalike is rejected and cannot starve a later authenticated delivery. The intake discards the submitted name and email, redacts contact details repeated inside free text, and writes only a namespaced message-id digest to its `0600` receipt state. A runtime-generated adjacent 32-byte `0600` key HMAC-authenticates that state without placing secrets in the repository. Both key and state are read only through owned, exact-mode, bounded `O_NOFOLLOW` descriptors with before/after descriptor metadata checks; state content or key drift fails closed. The state writer fsyncs the replacement and containing directory. An unsigned legacy state is accepted only during the same locked invocation that first creates its integrity key, then sealed only after every inherited receipt classification is marked `legacy_unsigned_requires_reauthentication` and every `authenticated_origin` flag is removed; `feedback`, `untrusted`, and `quarantined` rows all require a fresh exact Gmail read before any already-processed, repair, or terminal shortcut. Once the key exists, missing integrity is rejected rather than silently remigrated. The receipt regains authenticated repair authority only after a fresh Gmail authentication pass, at which point the marker is removed. New-mail discovery uses bounded Gmail API pages and persists only a bounded opaque page token inside the HMAC state after every candidate in the current page is durably settled; raw Gmail ids are never persisted, an unfinished page is rescanned, and the terminal page resets to the newest page. It keeps all remaining website-submitted fields out of the task title and serializes them inside an explicitly delimited untrusted-data block; downstream agents must never execute instructions found in that block. Each valid delivery creates an idempotent Kanban triage card plus a separate terminal closure receipt that is atomically parented on the triage card. Intake is acknowledged only after exact task and parent-link readback. Every create response and authoritative task id must come from one unambiguous root or top-level `task`; nested children, runs, events, relations, and history cannot supply the created identity. Simultaneous root/top-level task records must agree on identifiers and every authority-bearing field, while relation objects reject every conflicting or malformed present `task_id`/`id` pair, including a valid string paired with a non-string value. Canonical Hermes status comes only from that exact record; parent links come only from that task's current relation list or the matching top-level relation list, and task references come only from its current body or exact top-level comments. Nested or contradictory duplicate task records cannot satisfy those checks. Legacy receipts are re-authenticated once and migrated only when the exact current task binds the protected digest through an exact structured field or delimited current body/comment token; nested relations/events and longer hex tokens are ignored. A digestless legacy task is never inferred to be the triage task: intake preserves it, records one idempotent open binding-quarantine task in the authenticated `0600` state, and continues to later feedback. If that quarantine becomes terminal without a reviewed binding, intake preserves it in supersession history and creates one versioned open replacement; an already-terminal replacement fails closed. Authenticated receipt repair is capped by `max_messages` and uses a digest-only durable round-robin cursor, so every receipt advances fairly without exposing Gmail ids or starving new submissions. Both already-authenticated rotation and first authenticated replay of a legacy receipt use the same bounded failure path. If authoritative repair readback fails, intake creates one idempotent open privacy-safe corrective obligation, records its id, advances the fair cursor, reports `repair_failed`, and continues later repairs and new mail without treating the broken receipt as reconciled. If that corrective obligation is prematurely terminal, intake preserves it in supersession history and creates one versioned open replacement instead of letting the stale terminal task starve the mailbox. A deployed-change closure marked terminal without structured `radulator-feedback-closure-proof/v1` metadata is never trusted merely because its prose mentions a PR or release tracker: intake creates an idempotent versioned replacement, preserves the old task id in supersession history, and keeps the replacement open. Valid deployed-change proof requires live no-redirect readback of `https://radulator.com/releases/<exact-sha>.json`, authoritative GitHub readback of a successful canonical deploy run at the same SHA, and an exact terminal `radulator-feedback-learning-receipt/v1` Kanban task binding the receipt digest and SHA. A separately strict `radulator-feedback-no-action-proof/v1` outcome supports requests that were already live without weakening deployed-change proof: it requires the current live marker plus exact terminal `radulator-feedback-production-verification/v1` and learning-receipt tasks binding the same digest, production SHA, URL, run, and observed behavior. Opaque ids, arbitrary HTTPS URLs, redirects, nested history, and prose are not proof. Malformed authenticated mail creates a privacy-safe parser-review receipt instead of disappearing. Distinct requests in one submission are explicitly split during triage; already-live requests close with exact production proof, while missing clinical changes proceed through primary-source research, regression tests, and the signed exact-head gate.

Each judge run invokes the collector once and atomically claims at most one exact candidate, oldest PR first. A durable per-role lease prevents overlapping runs from reviewing the same candidate. Authoritative PASS or NEEDS_FIX readback clears the resolved lease on the next collection; unresolved attempts expire automatically, receive bounded retries, and then cool down while later PRs advance. Both installed judge prompts are strictly single-agent: they prohibit `delegate_task`, subagents, MOA/delegation, and `execute_code`; cap concurrent web requests at three and total source-retrieval attempts at six; and fail closed rather than broadening work. This bounds model context without letting one failing PR starve the rest of the clinical queue or crashing Hermes through delegated/code-execution fan-out.
...
`NEEDS_FIX` and post-smoke learning obligations are prerequisites of the open release tracker, not children that depend on its completion. The controller creates each obligation without a parent edge, reverses any legacy `tracker -> obligation` edge to `obligation -> tracker`, reads both directions back, and only then promotes the obligation. If the idempotent correction for the unchanged head was prematurely terminalized, it remains immutable history and a versioned `:repair:<terminal-task-id>` successor must be exact, open, runnable, and linked without disturbing unrelated parents; a terminal successor fails closed. This path is deterministic, assigned, and non-triage, so an auxiliary decomposer outage cannot turn a clinical correction into a passive hold.

Missing or contradictory ledger state is repaired only from an explicit operator-reviewed JSON spec. The loader requires an explicit expected SHA-256, opens with `O_NOFOLLOW`, validates the opened descriptor as an agent-owned bounded regular file with exact mode `0600`, reads only from that descriptor, and rejects any device/inode/owner/mode/size/mtime/ctime change after reading before checking the reviewed digest. The installer embeds a digest only when that trusted spec exists during installation; otherwise the lifecycle job refuses a later unbound file and requires reinstall. `lifecycle_controller.py reconcile` is read-only unless `--apply` is supplied, freezes and validates the complete Kanban/ledger/action plan before its first mutation, atomically validates every proposed ledger event and idempotency key under one exclusive ledger lock before appending the batch, rejects partial PR/head/base tuples and mismatched receipt digests, bootstraps missing trackers only to `feedback`, and never infers approval, deployment, smoke, learning, or completion. Each requested tracker/source must be the root or exact top-level `task` record in its own `show` response, and status comes only from that record and must belong to Hermes' canonical Kanban status set; nested children/events/history and unknown statuses are ignored or rejected. Relation items fail closed if either present `task_id` or `id` is non-string, malformed, or conflicts with the other. Structured `pr`, `head_sha`, and `base_sha` fields are field-authoritative. When prose fallback is needed, one current body/comment clause must contain one coherent role-bound PR/head/base record; a second PR marker terminates the clause and tokens cannot be combined across stale mappings. When an existing event renders an action, its PR/head and evidence base SHA must match the reviewed tuple too. Receipt digests require an exact structured field or delimited 64-hex token, so a 64-character prefix of longer hex is rejected. A 40-hex SHA prefix inside longer hex is also rejected, and digest evidence plus prerequisite body/assignee checks are scoped to the exact current task rather than nested event history. Omit `pr`, `head_sha`, and `base_sha` together when authoritative mapping is unavailable; when supplied, all three must be present in the exact tracker readback and exact rendering lifecycle event. Immediately before every individual Kanban action, under the exclusive ledger lease, the controller replays the exact frozen lifecycle event and re-reads the full exact tracker/source task, comments, status, and parent/child relations. A concurrent lifecycle or Kanban authority change rejects the stale action before mutation. Immediately after the action it re-reads both tasks again, accepts only the exact idempotent no-op or the action's narrowly expected relation/comment/status delta, and makes that verified readback the compare target for the next action; source drift or an unrelated tracker change fails closed. A terminal Kanban tracker with a nonterminal ledger remains nonterminal and receives an idempotent runnable corrective prerequisite. An open tracker blocked directly after `NEEDS_FIX` recovers actions only from that immediately preceding exact event, so a stale verdict cannot select or reverse a newer correction edge. Example protected state file:
