...
Use one entry per tracker. MELD and KBRC must omit `pr`, `head_sha`, and `base_sha` together unless the refreshed audit has authoritative readback for all three. Their missing ledger history bootstraps only to `feedback`. The KBRC source must name the exact authenticated receipt task and full 64-character receipt digest. CAC may include its PR/head/base only when all three are present in the refreshed tracker readback. A structured `pr`, `head_sha`, or `base_sha` field on the exact task is authoritative for that field: a conflicting value fails closed, even if stale body/comment prose contains the requested token. Delimited prose fallback is allowed only when the corresponding structured field is absent.
