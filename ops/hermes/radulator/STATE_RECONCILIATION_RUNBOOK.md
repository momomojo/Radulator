...
Read the receipt state and both Kanban cards back. The new closure must be open, must depend on `t_1630667d`, and must not be terminal until its exact completed run contains one of two non-interchangeable proofs. A deployed change uses `radulator-feedback-closure-proof/v1` metadata binding the receipt digest to one immutable release-marker SHA, the same production-smoke SHA/run, and the retained-learning receipt id. A request verified as already live uses `radulator-feedback-no-action-proof/v1`, binding the digest to the current production SHA and exact Radulator release-marker URL, a direct trusted Radulator verification URL/run, bounded observed behavior, and retained learning. A PR, audit, delegated tracker, prose-only claim, or non-Radulator URL is not closure. Re-running intake must create no additional task.
...
The plan must show only `feedback` bootstrap for missing MELD/KBRC coverage, a corrective prerequisite for archived/nonterminal CAC, and—when the refreshed ledger still shows it—the exact retained `NEEDS_FIX` recovery for `t_45a5323f`. It must not show approval, merge, deploy, smoke, learning, or completion. The controller freezes and validates every tracker, source, digest, optional PR/head/base tuple, existing ledger source/digest, and rendered action before any apply-side mutation, so one invalid later entry leaves the whole batch unapplied. PR `#16` must never match `#169`; authority and current prerequisite body/assignee must come from the exact task record, not historical events or related tasks. Recheck the frozen spec digest, then repeat the same command with `--apply`. Repeat it once more: it must report idempotent action readback and create no duplicate events, tasks, comments, or dependency edges.
