...
Use one entry per tracker. MELD and KBRC must omit `pr`, `head_sha`, and `base_sha` together unless the refreshed audit has authoritative readback for all three. Their missing ledger history bootstraps only to `feedback`. The KBRC source must name the exact authenticated receipt task and full 64-character receipt digest. The source readback must contain that digest in an exact structured `receipt_digest`/`digest` field or as a delimited 64-hex token; a prefix of longer hex is not authority. CAC may include its PR/head/base only when all three are present in the refreshed tracker readback. A structured `pr`, `head_sha`, or `base_sha` field on the exact task is authoritative for that field: a conflicting value fails closed, even if stale body/comment prose contains the requested token. If a field is absent, fallback requires one current body/comment line containing one coherent role-bound `PR #<n> ... head <40hex> ... base <40hex>` record matching the whole tuple. Never combine tokens from separate or stale mappings.
