...
Current authority contract: authenticated Formspree repair rotates over the privacy-minimized durable receipt set, while new intake advances through bounded Gmail API pages with an authenticated opaque page token, so repeated runs fetch message 101 and later without persisting raw ids. Reconciliation apply holds one exclusive lifecycle-ledger lease across the complete external action plan; under that lease it compares exact frozen ledger and tracker/source Kanban authority immediately before and after every action, rejects stale preflight authority before mutation, accepts only the expected action delta, and blocks later ledger appends until release.
...
Each legacy delivery is re-read from Gmail and must pass aligned Formspree authentication before any migration. One-time unsigned-state migration marks every inherited `feedback`, `untrusted`, or `quarantined` classification as requiring reauthentication and removes inherited origin authority; no already-processed, repair, or terminal shortcut may run until the exact Gmail delivery is fetched and re-evaluated. When the exact persisted digest is present on the legacy Kanban task, intake preserves that task in `superseded_task_ids`, assigns it as `triage_task_id`, and creates one idempotent open replacement closure whose key is versioned by the old task id. It persists the new closure as `task_id` only after digest, parent, status, and task-id readback. Task identity and canonical status must come from the exact root/top-level `task`; parent links must come from that task's current relation list or its matching top-level relation list, and task references must come from its current body or exact top-level comments. Every present relation `task_id`/`id` pair must contain the same valid string identifier; a conflicting or non-string companion fails closed. Nested children, events, runs, history, relations, or duplicate task records are never authority.
...
Authenticated receipt repair is bounded separately from new intake. At most `--max-messages` authenticated receipts receive Kanban repair readback per run, and the `0600` HMAC state persists only the last namespaced receipt digest as `authenticated_reconciliation_cursor`. Repeated runs rotate across every authenticated receipt while new mail continues independently. New discovery reads one bounded Gmail API page at a time; it persists only the bounded opaque `gmail_search_page_token` after every candidate in the current page is durably settled, rescans an unfinished page, and removes the token after the terminal page so discovery returns to newest mail. Raw Gmail ids, message content, senders, and subjects are never persisted. Both the authenticated rotation and the first authenticated replay of a legacy receipt use the same bounded failure path. If a receipt's durable task/readback is broken, intake must create/read back one idempotent open `radulator-formspree-reconciliation-failure:<digest>` obligation, persist its task id, increment `repair_failed`, advance the cursor, and continue. That outcome is not `reconciled`. If the persisted failure obligation is prematurely terminal, preserve its id in `superseded_reconciliation_failure_task_ids` and create/read back the versioned open `radulator-formspree-reconciliation-failure-repair:<digest>:<old-task-id>` replacement; the stale terminal task must not starve later repairs or new mail. Failure to create the open corrective obligation remains fail-closed.
