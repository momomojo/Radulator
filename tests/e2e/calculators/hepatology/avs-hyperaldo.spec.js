...
test.describe('AVS Hyperaldo Calculator', () => {
  test.beforeEach(async ({ page }) => {
    await navigateToCalculator(page, 'AVS – Aldosterone (PA)');

    // Verify calculator loaded
    await expect(page.locator('h2:has-text("AVS – Aldosterone (PA)")')).toBeVisible();
    await expect(page.locator('text=Comprehensive primary aldosteronism AVS')).toBeVisible();
  });
  ...
  test.describe('Visual Appeal & Theme Matching', () => {
    ...
    test('should be responsive on mobile devices', async ({ page }) => {
      ...
      const patientInfoGrid = page.locator('.grid.grid-cols-1.md\\:grid-cols-2').first();
    ...
    test('should display all required input sections', async ({ page }) => {
      ...
      // Unit selection
      await expect(page.locator('label:has-text("Aldosterone Units")')).toBeVisible();
      await expect(page.locator('label:has-text("Cortisol Units")')).toBeVisible();
  ...
  test.describe('User Usefulness - Clinical Scenarios', () => {
    test('Scenario 1: Unilateral left aldosteronism (post-ACTH)', async ({ page }) => {
      ...
      await page.locator('label:has-text("Infrarenal IVC Aldosterone")').locator('~ input').fill('85');
      await page.locator('label:has-text("Infrarenal IVC Cortisol")').locator('~ input').fill('18');

      // Fill Left AV sample 1
      await page.locator('h4:has-text("Left Adrenal Vein")').locator('~ div').first().locator('input[type="number"]').first().fill('2900');
      await page.locator('h4:has-text("Left Adrenal Vein")').locator('~ div').first().locator('input[type="number"]').nth(1).fill('280');

      // Fill Right AV sample 1
      await page.locator('h4:has-text("Right Adrenal Vein")').locator('~ div').first().locator('input[type="number"]').first().fill('450');
      await page.locator('h4:has-text("Right Adrenal Vein")').locator('~ div').first().locator('input[type="number"]').nth(1).fill('320');

      // Calculate
      await page.locator('button:has-text("Calculate")').click();

      // Verify results
      await expect(page.locator('text=Post-ACTH Results')).toBeVisible({ timeout: 2000 });
      await expect(page.locator('text=/Bilateral successful cannulation/i')).toBeVisible();
      await expect(page.locator('text=/Unilateral aldosterone hypersecretion on Left side/i')).toBeVisible();

      // Verify LI > 4 (post-ACTH threshold)
      const liText = await page.locator('text=/LI:.*\\d+\\.\\d+/').textContent();
      const liValue = parseFloat(liText.match(/LI:\s*([\d.]+)/)[1]);
      expect(liValue).toBeGreaterThan(4);

      // Verify CSI/RASI criteria visible
      await expect(page.locator('text=Chow 2024 Unilateral Criteria')).toBeVisible();
    ...
    test('Scenario 2: Bilateral disease (equivocal LI)', async ({ page }) => {
      ...
      // Fill IVC measurements
      await page.locator('label:has-text("Infrarenal IVC Aldosterone")').locator('~ input').fill('90');
      await page.locator('label:has-text("Infrarenal IVC Cortisol")').locator('~ input').fill('20');

      // Fill Left AV sample - similar to right (bilateral)
      await page.locator('h4:has-text("Left Adrenal Vein")').locator('~ div').first().locator('input[type="number"]').first().fill('800');
      await page.locator('h4:has-text("Left Adrenal Vein")').locator('~ div').first().locator('input[type="number"]').nth(1).fill('250');

      // Fill Right AV sample - similar to left (bilateral)
      await page.locator('h4:has-text("Right Adrenal Vein")').locator('~ div').first().locator('input[type="number"]').first().fill('750');
      await page.locator('h4:has-text("Right Adrenal Vein")').locator('~ div').first().locator('input[type="number"]').nth(1).fill('240');
      ...
      await expect(page.locator('text=Post-ACTH Results')).toBeVisible({ timeout: 2000 });
      await expect(page.locator('text=/Bilateral disease likely/i')).toBeVisible();

      // Take screenshot
      await takeScreenshot(page, 'avs-hyperaldo', 'bilateral-disease');
    });

    test('Scenario 3: Cannulation failure with CSI/RASI rescue', async ({ page }) => {
      // Select post-ACTH protocol
      await page.locator('input[type="radio"][value="post"]').check();

      // Fill IVC measurements
      await page.locator('label:has-text("Infrarenal IVC Aldosterone")').locator('~ input').fill('100');
      await page.locator('label:has-text("Infrarenal IVC Cortisol")').locator('~ input').fill('25');

      // Fill Left AV sample - good cannulation (SI > 5)
      await page.locator('h4:has-text("Left Adrenal Vein")').locator('~ div').first().locator('input[type="number"]').first().fill('3500');
      await page.locator('h4:has-text("Left Adrenal Vein")').locator('~ div').first().locator('input[type="number"]').nth(1).fill('350');

      // Fill Right AV sample - poor cannulation (SI < 5)
      await page.locator('h4:has-text("Right Adrenal Vein")').locator('~ div').first().locator('input[type="number"]').first().fill('150');
      await page.locator('h4:has-text("Right Adrenal Vein")').locator('~ div').first().locator('input[type="number"]').nth(1).fill('80');

      // Calculate
      await page.locator('button:has-text("Calculate")').click();

      // Verify cannulation failure warning
      await expect(page.locator('text=/Cannulation failure/i')).toBeVisible({ timeout: 2000 });

      // Verify CSI/RASI rescue criteria mentioned
      await expect(page.locator('text=/unilateral-cannulating criteria/i')).toBeVisible();
    ...
    test('Scenario 4: Pre and Post-ACTH comparison', async ({ page }) => {
      ...
      // Fill Pre-ACTH data
      await page.locator('text=Pre-Cosyntropin Protocol').locator('..').locator('label:has-text("Infrarenal IVC Aldosterone")').locator('~ input').fill('75');
      await page.locator('text=Pre-Cosyntropin Protocol').locator('..').locator('label:has-text("Infrarenal IVC Cortisol")').locator('~ input').fill('15');

      const preLeftAldoInputs = await page.locator('text=Pre-Cosyntropin Protocol').locator('..').locator('h4:has-text("Left Adrenal Vein")').locator('~ div').first().locator('input[type="number"]').all();
      await preLeftAldoInputs[0].fill('1800');
      await preLeftAldoInputs[1].fill('180');

      const preRightAldoInputs = await page.locator('text=Pre-Cosyntropin Protocol').locator('..').locator('h4:has-text("Right Adrenal Vein")').locator('~ div').first().locator('input[type="number"]').all();
      await preRightAldoInputs[0].fill('350');
      await preRightAldoInputs[1].fill('200');

      // Fill Post-ACTH data
      await page.locator('text=Post-Cosyntropin Protocol').locator('..').locator('label:has-text("Infrarenal IVC Aldosterone")').locator('~ input').fill('85');
      await page.locator('text=Post-Cosyntropin Protocol').locator('..').locator('label:has-text("Infrarenal IVC Cortisol")').locator('~ input').fill('18');

      const postLeftAldoInputs = await page.locator('text=Post-Cosyntropin Protocol').locator('..').locator('h4:has-text("Left Adrenal Vein")').locator('~ div').first().locator('input[type="number"]').all();
      await postLeftAldoInputs[0].fill('2900');
      await postLeftAldoInputs[1].fill('280');

      const postRightAldoInputs = await page.locator('text=Post-Cosyntropin Protocol').locator('..').locator('h4:has-text("Right Adrenal Vein")').locator('~ div').first().locator('input[type="number"]').all();
      ...
      await expect(page.locator('text=Pre-ACTH Results')).toBeVisible({ timeout: 2000 });
      await expect(page.locator('text=Post-ACTH Results')).toBeVisible();

      // Verify both show results in grid layout
      const comparisonGrid = page.locator('.grid.grid-cols-1.md\\:grid-cols-2').last();
    ...
    test('Scenario 5: Multi-sample averaging (2 left, 4 right)', async ({ page }) => {
      // Select post-ACTH protocol
      await page.locator('input[type="radio"][value="post"]').check();
      ...
      await page.locator('h4:has-text("Left Adrenal Vein")').locator('~ button:has-text("+ Add Sample")').click();

      // Fill Left AV samples (2 samples)
      const leftInputs = await page.locator('h4:has-text("Left Adrenal Vein")').locator('~ div').locator('input[type="number"]').all();
      ...
      await page.locator('h4:has-text("Right Adrenal Vein")').locator('~ button:has-text("+ Add Sample")').click();
      await page.locator('h4:has-text("Right Adrenal Vein")').locator('~ button:has-text("+ Add Sample")').click();
      await page.locator('h4:has-text("Right Adrenal Vein")').locator('~ button:has-text("+ Add Sample")').click();

      // Fill Right AV samples (4 samples)
      const rightInputs = await page.locator('h4:has-text("Right Adrenal Vein")').locator('~ div').locator('input[type="number"]').all();
    ...
    test('Scenario 6: Unit conversion (pg/mL and nmol/L)', async ({ page }) => {
      ...
      await page.locator('label:has-text("Infrarenal IVC Aldosterone")').locator('~ input').fill('850'); // 85 ng/dL = 850 pg/mL
      await page.locator('label:has-text("Infrarenal IVC Cortisol")').locator('~ input').fill('496.62'); // 18 µg/dL ≈ 496.62 nmol/L

      // Fill Left AV sample (in alternative units)
      await page.locator('h4:has-text("Left Adrenal Vein")').locator('~ div').first().locator('input[type="number"]').first().fill('29000'); // 2900 ng/dL
      await page.locator('h4:has-text("Left Adrenal Vein")').locator('~ div').first().locator('input[type="number"]').nth(1).fill('7725.2'); // 280 µg/dL

      // Fill Right AV sample (in alternative units)
      await page.locator('h4:has-text("Right Adrenal Vein")').locator('~ div').first().locator('input[type="number"]').first().fill('4500'); // 450 ng/dL
      await page.locator('h4:has-text("Right Adrenal Vein")').locator('~ div').first().locator('input[type="number"]').nth(1).fill('8828.8'); // 320 µg/dL
      ...
      await expect(page.locator('text=Post-ACTH Results')).toBeVisible({ timeout: 2000 });
      await expect(page.locator('text=/Unilateral aldosterone hypersecretion/i')).toBeVisible();
  ...
  test.describe('Edge Cases & Validation', () => {
    test('should warn on extreme aldosterone values', async ({ page }) => {
      ...
      await page.locator('label:has-text("Infrarenal IVC Aldosterone")').locator('~ input').fill('600'); // > 500
      await page.locator('label:has-text("Infrarenal IVC Cortisol")').locator('~ input').fill('18');

      // Fill normal adrenal vein samples
      await page.locator('h4:has-text("Left Adrenal Vein")').locator('~ div').first().locator('input[type="number"]').first().fill('15000'); // > 10000
      await page.locator('h4:has-text("Left Adrenal Vein")').locator('~ div').first().locator('input[type="number"]').nth(1).fill('280');
      ...
      await expect(page.locator('text=Value Validation Warnings')).toBeVisible({ timeout: 2000 });
      await expect(page.locator('text=/IVC Aldosterone.*extremely high/i')).toBeVisible();
    ...
    test('should detect conflicting criteria', async ({ page }) => {
      ...
      await page.locator('h4:has-text("Left Adrenal Vein")').locator('~ div').first().locator('input[type="number"]').first().fill('1500');
      await page.locator('h4:has-text("Left Adrenal Vein")').locator('~ div').first().locator('input[type="number"]').nth(1).fill('250');

      await page.locator('h4:has-text("Right Adrenal Vein")').locator('~ div').first().locator('input[type="number"]').first().fill('500');
      await page.locator('h4:has-text("Right Adrenal Vein")').locator('~ div').first().locator('input[type="number"]').nth(1).fill('240');
      ...
      const conflictSection = page.locator('text=Conflicting Criteria Detected');
      const isVisible = await conflictSection.isVisible({ timeout: 1000 }).catch(() => false);
    ...
    test('should handle missing data gracefully', async ({ page }) => {
      ...
      await expect(page.locator('text=/Insufficient data/i')).toBeVisible({ timeout: 2000 });
    });
    ...
    test('should handle incomplete comparison view data', async ({ page }) => {
      ...
      const postLeftInputs = await page.locator('text=Post-Cosyntropin Protocol').locator('..').locator('h4:has-text("Left Adrenal Vein")').locator('~ div').first().locator('input[type="number"]').all();
      await postLeftInputs[0].fill('2900');
      await postLeftInputs[1].fill('280');

      const postRightInputs = await page.locator('text=Post-Cosyntropin Protocol').locator('..').locator('h4:has-text("Right Adrenal Vein")').locator('~ div').first().locator('input[type="number"]').all();
      ...
      await expect(page.locator('text=/Incomplete Data for Comparison View/i')).toBeVisible({ timeout: 2000 });
    });
  ...
  test.describe('Reference URL Validation', () => {
    test('should have all reference links present', async ({ page }) => {
      ...
      const referenceLinks = await page.locator('section:has(h3:has-text("References")) a[href^="http"]').all();
  ...
  test.describe('Professional Appearance', () => {
    test('should display clear clinical guidance', async ({ page }) => {
      ...
      await expect(page.locator('text=/All calculations are performed using standard units/i')).toBeVisible();

      // Verify unit conversion help text
      await expect(page.locator('text=/1 ng\\/dL = 10 pg\\/mL/i')).toBeVisible();
      await expect(page.locator('text=/1 µg\\/dL = 27\\.59 nmol\\/L/i')).toBeVisible();
    });
    ...
    test('should use medical terminology correctly', async ({ page }) => {
      ...
      await expect(page.locator('text=/Contralateral Suppression/i')).toBeVisible();
      await expect(page.locator('text=/ACTH/i')).toBeVisible();
    });

    test('should provide comprehensive calculation outputs', async ({ page }) => {
      ...
      await expect(page.locator('text=Left SI:')).toBeVisible({ timeout: 2000 });
