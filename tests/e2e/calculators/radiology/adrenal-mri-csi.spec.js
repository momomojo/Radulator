...
/**
 * E2E Tests for Adrenal MRI Chemical Shift Calculator
 *
...
const CALCULATOR_NAME = "Adrenal MRI Chemical Shift";

test.describe("Adrenal MRI Chemical Shift Calculator - Core Functionality", () => {
  test.beforeEach(async ({ page }) => {
    await navigateToCalculator(page, CALCULATOR_NAME);
  });

  test("should load calculator with correct title and description", async ({
    page,
  }) => {
  ...
  test("should display all required input fields", async ({ page }) => {
    ...
    await expect(
      page.locator('label:has-text("Adrenal SI opposed‑phase")'),
    ).toBeVisible();
    await expect(
      page.locator('label:has-text("Spleen SI in‑phase")'),
    ).toBeVisible();
    await expect(
      page.locator('label:has-text("Spleen SI opposed‑phase")'),
    ).toBeVisible();
...
test.describe("Adrenal MRI Chemical Shift Calculator - Clinical Test Cases", () => {
  test.beforeEach(async ({ page }) => {
    await navigateToCalculator(page, CALCULATOR_NAME);
  });

  test("Test Case 1: Lipid-rich Adenoma (Classic Positive)", async ({
    page,
  }) => {
  ...
  test("Test Case 4: High signal intensity drop (strong adenoma)", async ({
    page,
  }) => {
...
test.describe("Adrenal MRI Chemical Shift Calculator - Edge Cases & Validation", () => {
  test.beforeEach(async ({ page }) => {
    await navigateToCalculator(page, CALCULATOR_NAME);
  });
  ...
  test("should clear results when switching calculators", async ({ page }) => {
    ...
    // Input fields should be empty - use getByRole for reliable selection
    const firstInput = page.getByRole("spinbutton", {
      name: /Adrenal SI in‑phase/i,
    });
    await expect(firstInput).toHaveValue("");
  });
});

test.describe("Adrenal MRI Chemical Shift Calculator - Formula Verification", () => {
  test.beforeEach(async ({ page }) => {
    await navigateToCalculator(page, CALCULATOR_NAME);
  });

  test("should verify Signal Intensity Index formula accuracy", async ({
    page,
  }) => {
...
test.describe("Adrenal MRI Chemical Shift Calculator - UI/UX Quality", () => {
  test.beforeEach(async ({ page }) => {
    await navigateToCalculator(page, CALCULATOR_NAME);
  });
...
test.describe("Adrenal MRI Chemical Shift Calculator - References & Documentation", () => {
  test.beforeEach(async ({ page }) => {
    await navigateToCalculator(page, CALCULATOR_NAME);
  });
  ...
  test("should display reference section", async ({ page }) => {
    ...
    await expect(
      page.locator('a:has-text("Schieda N AJR 2017")'),
    ).toBeVisible();
  });
...
test.describe("Adrenal MRI Chemical Shift Calculator - Performance & Browser Compatibility", () => {
  ...
  test("should have no console errors during normal use", async ({ page }) => {
    ...
    await expect(page.getByRole('status', { name: 'Calculator results' }).first()).toBeVisible();

    // Should have no console errors
    expect(consoleErrors).toHaveLength(0);
  });

  test("should make no external network requests during calculation", async ({
    page,
  }) => {
...
test.describe("Adrenal MRI Chemical Shift Calculator - Clinical Scenarios", () => {
  test.beforeEach(async ({ page }) => {
    await navigateToCalculator(page, CALCULATOR_NAME);
  });

  test("Clinical Scenario: Incidental adrenal mass - confirm adenoma", async ({
    page,
  }) => {
  ...
  test("Clinical Scenario: Possible metastasis - indeterminate lesion", async ({
    page,
  }) => {
    ...
    await takeScreenshot(
      page,
      "adrenal-mri-csi",
      "clinical-indeterminate-needs-workup",
    );
  });

  test("Clinical Scenario: Borderline case requiring correlation", async ({
    page,
  }) => {
    ...
    await takeScreenshot(
      page,
      "adrenal-mri-csi",
      "clinical-borderline-needs-correlation",
    );
  });
});
