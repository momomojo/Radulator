...
test.describe('Adrenal CT Washout Calculator', () => {
  ...
  test('should display calculator with correct title and description', async ({ page }) => {
    // Verify title
    ...
    // Verify info box with caveats
    const infoBox = page.locator('.bg-blue-50\\/60');
    await expect(infoBox).toBeVisible();
    await expect(infoBox).toContainText('Caveat');
    await expect(infoBox).toContainText('hypervascular extraadrenal primary tumors');
  ...
  test('should have all required input fields', async ({ page }) => {
    ...
    await expect(page.locator('label:has-text("Pre‑contrast HU")')).toBeVisible();
    await expect(page.locator('label:has-text("Post‑contrast HU (60‑75 s)")')).toBeVisible();
    await expect(page.locator('label:has-text("Delayed HU (15 min)")')).toBeVisible();

    // Verify Calculate button is present
    await expect(page.locator('button:has-text("Calculate")')).toBeVisible();
  });

  test('Test Case 1: Typical Adenoma (both washouts meet thresholds)', async ({ page }) => {
    ...
    // Wait for results
    await page.waitForSelector('section[aria-live="polite"]', { state: 'visible' });
  ...
  test('Test Case 3: Borderline adenoma (absolute meets, relative does not)', async ({ page }) => {
  ...
  test('Test Case 4: Borderline adenoma (relative meets, absolute does not)', async ({ page }) => {
  ...
  test('Test Case 5: High enhancement lesion (typical metastasis pattern)', async ({ page }) => {
  ...
  test('Test Case 6: Lipid-poor adenoma (still meets criteria)', async ({ page }) => {
  ...
  test('Edge Case: Negative HU values (lipid-rich adenoma)', async ({ page }) => {
  ...
  test('should verify all reference citations are accessible', async ({ page }) => {
    ...
    await expect(page.locator('a:has-text("Caoili EM AJR 2000")')).toBeVisible();
    await expect(page.locator('a:has-text("Choi YA Radiology 2013")')).toBeVisible();
    await expect(page.locator('a:has-text("Park SY Abdom Imaging 2015")')).toBeVisible();
