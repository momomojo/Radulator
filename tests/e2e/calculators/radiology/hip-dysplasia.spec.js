...
test.describe('Hip Dysplasia Indices Calculator', () => {
  test.beforeEach(async ({ page }) => {
    await page.goto('/');
    // Click on Hip Dysplasia calculator in sidebar
  ...
  test('should display calculator name and description', async ({ page }) => {
    ...
    await expect(page.locator('text=Calculate migration indices and normal values in hip dysplasia')).toBeVisible();
  });

  test('should display info section with migration index explanation', async ({ page }) => {
    // Check for info text
    await expect(page.locator('text=Migration index measurements')).toBeVisible();
    await expect(page.locator('text=a = lateral distance from femoral head to acetabular line')).toBeVisible();
    await expect(page.locator('text=b = medial distance from acetabular line to center of femoral head')).toBeVisible();
  ...
  test('should display all input fields', async ({ page }) => {
    ...
    await expect(page.locator('label:has-text("Migration Index")')).toHaveCount(4);
  });

  test('should calculate normal values for newborn female (0-3 months)', async ({ page }) => {
  ...
  test('should calculate normal values for newborn male (0-3 months)', async ({ page }) => {
  ...
  test('should calculate normal values for infant female (4-6 months)', async ({ page }) => {
  ...
  test('should calculate normal values for infant male (4-6 months)', async ({ page }) => {
  ...
  test('should calculate normal values for 1-year-old female', async ({ page }) => {
  ...
  test('should calculate normal values for 2-year-old male', async ({ page }) => {
  ...
  test('should calculate normal values for preschool child (3-5 years)', async ({ page }) => {
  ...
  test('should calculate normal values for school age child (>5 years)', async ({ page }) => {
  ...
  test('should calculate migration index for right hip - normal range', async ({ page }) => {
  ...
  test('should calculate migration index - borderline/at risk (22-32%)', async ({ page }) => {
    ...
    await expect(page.locator('text=Borderline/At risk')).toBeVisible();
  });

  test('should calculate migration index - subluxation (33-59%)', async ({ page }) => {
    ...
    await expect(page.locator('text=Subluxation')).toBeVisible();
  });

  test('should calculate migration index - severe subluxation (60-89%)', async ({ page }) => {
    ...
    await expect(page.locator('text=Severe subluxation')).toBeVisible();
  });

  test('should calculate migration index - dislocation (≥90%)', async ({ page }) => {
    ...
    await expect(page.locator('text=Dislocation')).toBeVisible();
  });
  ...
  test('should handle infant under 3 years with different interpretation ranges', async ({ page }) => {
  ...
  test('should handle edge case: 100% migration (complete dislocation)', async ({ page }) => {
    const threeYearsAgo = new Date();
    threeYearsAgo.setFullYear(threeYearsAgo.getFullYear() - 3);
    const dobString = threeYearsAgo.toISOString().split('T')[0];
  ...
  test('should handle edge case: 0% migration (perfectly centered)', async ({ page }) => {
    const fourYearsAgo = new Date();
    fourYearsAgo.setFullYear(fourYearsAgo.getFullYear() - 4);
    const dobString = fourYearsAgo.toISOString().split('T')[0];
  ...
  test('should display all three references', async ({ page }) => {
    ...
    await expect(page.locator('a[href="https://pubmed.ncbi.nlm.nih.gov/954321/"]')).toBeVisible();
    await expect(page.locator('text=Tönnis D. Normal values of the hip joint')).toBeVisible();

    await expect(page.locator('a[href="https://doi.org/10.1007/978-3-662-06621-8"]')).toBeVisible();
    await expect(page.locator('text=Die Angeborene Hüftdysplasie')).toBeVisible();

    await expect(page.locator('a[href="https://pubmed.ncbi.nlm.nih.gov/6930145/"]')).toBeVisible();
    await expect(page.locator('text=Reimers J. The stability of the hip in children')).toBeVisible();
  });

  test('should open reference links in new tab', async ({ page }) => {
    const firstRef = page.locator('a[href="https://pubmed.ncbi.nlm.nih.gov/954321/"]');
    await expect(firstRef).toHaveAttribute('target', '_blank');
    await expect(firstRef).toHaveAttribute('rel', 'noopener noreferrer');
  });

  test('should require date of birth and gender for normal values', async ({ page }) => {
    await page.click('button:has-text("Calculate")');

    // Should show message when DOB or gender is missing
    await expect(page.locator('text=Enter date of birth and gender')).toBeVisible();
  });

  test('should calculate migration index without date of birth', async ({ page }) => {
