...
test.describe("Prostate Volume Calculator", () => {
  ...
  test.describe("Visual Appeal & Theme Matching", () => {
    test("should display calculator with proper styling", async ({ page }) => {
      ...
      const header = page.locator(
        'h2:has-text("Prostate Volume & PSA Density")',
      );
      await expect(header).toBeVisible();
      await expect(header).toHaveClass(/text-xl|font-semibold/);

      // Check description is visible
      const description = page.locator("text=Ellipsoid volume estimation").first();
      await expect(description).toBeVisible();
    });

    test("should display info box with formula explanation", async ({
      page,
    }) => {
    ...
    test("should have responsive layout on desktop", async ({
      page,
      viewport,
    }) => {
  ...
  test.describe("User Usefulness Assessment", () => {
    test("should display clear field labels", async ({ page }) => {
      await expect(
        page.locator('label:has-text("Length (craniocaudal, cm):")'),
      ).toBeVisible();
      await expect(
        page.locator('label:has-text("Height (anteroposterior, cm):")'),
      ).toBeVisible();
      await expect(
        page.locator('label:has-text("Width (transverse, cm):")'),
      ).toBeVisible();
      await expect(
        page.locator('label:has-text("PSA (ng/mL):")'),
      ).toBeVisible();
    });
  ...
  test.describe("Citation Verification", () => {
    ...
    test("should have valid Aminsharifi 2018 J Urol reference", async ({
      page,
    }) => {
      const link = page.locator(
        'a[href="https://doi.org/10.1016/j.juro.2018.05.016"]',
      );
      await expect(link).toBeVisible();
  ...
  test.describe("Professional Appearance", () => {
    ...
    test("should display results section with proper formatting", async ({
      page,
    }) => {
      // Fill in test values
  ...
  test.describe("Calculation Accuracy", () => {
    test("should calculate volume correctly - normal prostate", async ({
      page,
    }) => {
  ...
  test.describe("Clinical Interpretation", () => {
    ...
    test("should not display normal message for enlarged prostate", async ({
      ...
      await expect(
        page.locator("text=Normal prostate volume").first(),
      ).not.toBeVisible();
    });
