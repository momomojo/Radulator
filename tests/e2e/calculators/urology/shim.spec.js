...
test.describe('SHIM Score Calculator', () => {
  ...
  test.beforeEach(async ({ page }) => {
    ...
    await expect(page.locator('text=Sexual Health Inventory for Men')).toBeVisible();
  });
  ...
  test('should display all 5 SHIM questions', async ({ page }) => {
    ...
    await expect(page.locator('text=1. How do you rate your confidence')).toBeVisible();
    ...
    await expect(page.locator('text=2. When you had erections with sexual stimulation')).toBeVisible();
    await expect(page.locator('text=hard enough for penetration')).toBeVisible();
    await expect(page.locator('text=No sexual activity')).toBeVisible();

    // Question 3: Maintenance
    await expect(page.locator('text=3. During sexual intercourse, how often were you able to maintain')).toBeVisible();
    await expect(page.locator('text=Did not attempt intercourse')).toBeVisible();

    // Question 4: Difficulty maintaining
    await expect(page.locator('text=4. During sexual intercourse, how difficult was it to maintain')).toBeVisible();
    ...
    await expect(page.locator('text=5. When you attempted sexual intercourse, how often was it satisfactory')).toBeVisible();
  ...
  test('should display correct score range format', async ({ page }) => {
    ...
    await selectRadio(page, 'hard enough for penetration', 'Sometimes (about half the time)');
    await selectRadio(page, 'able to maintain', 'Sometimes (about half the time)');
    await selectRadio(page, 'how difficult', 'Difficult');
    await selectRadio(page, 'how often was it satisfactory', 'Sometimes (about half the time)');
  ...
  testCases.testCases.forEach((testCase) => {
    test(`${testCase.name} - ${testCase.notes}`, async ({ page }) => {
      ...
      await selectRadio(page, 'how often was it satisfactory', questionLabels.q5);
      ...
      await expect(resultsSection).toContainText(`Total SHIM Score: ${testCase.expected['Total SHIM Score']}`);

      // Verify ED Severity
      await expect(resultsSection).toContainText(`ED Severity: ${testCase.expected['ED Severity']}`);

      // Verify Clinical Interpretation
      await expect(resultsSection).toContainText(testCase.expected['Clinical Interpretation']);

      // Verify Note is present
      await expect(resultsSection).toContainText('Assessment based on past 6 months');
      await expect(resultsSection).toContainText('Score ≤21 indicates some degree of erectile dysfunction');
    });
  });

  test('Severe ED (score 5) - should show comprehensive evaluation recommendation', async ({ page }) => {
    // All questions at minimum (1 point each)
    await selectRadio(page, 'confidence', 'Very low');
    await selectRadio(page, 'hard enough for penetration', 'Almost never or never');
    await selectRadio(page, 'able to maintain', 'Almost never or never');
    await selectRadio(page, 'how difficult', 'Extremely difficult');
    await selectRadio(page, 'how often was it satisfactory', 'Almost never or never');
    ...
    await expect(resultsSection).toContainText('Comprehensive medical evaluation recommended');
  ...
  test('No ED (score 25) - should show normal function', async ({ page }) => {
    ...
    await selectRadio(page, 'hard enough for penetration', 'Almost always or always');
    await selectRadio(page, 'able to maintain', 'Almost always or always');
    await selectRadio(page, 'how difficult', 'Not difficult');
    await selectRadio(page, 'how often was it satisfactory', 'Almost always or always');
    ...
    await expect(resultsSection).toContainText('No evidence of erectile dysfunction');
    await expect(resultsSection).toContainText('Erectile function is normal');
  });

  test('Boundary test: Score 21 (Mild ED) vs Score 22 (No ED)', async ({ page }) => {
    // Test score 21 - should be Mild ED
    await selectRadio(page, 'confidence', 'Very high');
    await selectRadio(page, 'hard enough for penetration', 'Most times (much more than half the time)');
    await selectRadio(page, 'able to maintain', 'Most times (much more than half the time)');
    await selectRadio(page, 'how difficult', 'Most times (much more than half the time)');
    await selectRadio(page, 'how often was it satisfactory', 'Most times (much more than half the time)');
  ...
  test('Boundary test: Score 16 (Mild-to-Moderate) vs Score 17 (Mild)', async ({ page }) => {
    ...
    await expect(resultsSection).toContainText('PDE5 inhibitors are often effective');
  ...
  test('Boundary test: Score 11 (Moderate) vs Score 12 (Mild-to-Moderate)', async ({ page }) => {
    // Test score 11 - should be Moderate ED
    await selectRadio(page, 'confidence', 'Moderate');
    await selectRadio(page, 'hard enough for penetration', 'A few times (much less than half the time)');
    await selectRadio(page, 'able to maintain', 'A few times (much less than half the time)');
    await selectRadio(page, 'how difficult', 'Very difficult');
    await selectRadio(page, 'how often was it satisfactory', 'A few times (much less than half the time)');
  ...
  test('Boundary test: Score 7 (Severe) vs Score 8 (Moderate)', async ({ page }) => {
  ...
  test('No sexual activity responses - should handle zeros correctly', async ({ page }) => {
    // Q1 = 1, Q2-Q5 = 0 (no sexual activity)
    await selectRadio(page, 'confidence', 'Very low');
    await selectRadio(page, 'hard enough for penetration', 'No sexual activity');
    ...
    await selectRadio(page, 'how often was it satisfactory', 'Did not attempt intercourse');
    ...
    await expect(resultsSection).toContainText('Severe ED');
    await expect(resultsSection).toContainText('may include no sexual activity');
    await expect(resultsSection).toContainText('cardiovascular and endocrine assessment');
  });

  test('should verify all 4 reference links are present and valid', async ({ page }) => {
    ...
    const referenceLinks = await referencesSection.locator('a[href^="http"]').all();
    expect(referenceLinks.length).toBe(4);

    // Verify specific references
    await expect(referencesSection).toContainText('Rosen RC, et al. The International Index of Erectile Function');
  ...
  test('should maintain theme consistency', async ({ page }) => {
    ...
    const buttonColor = await calculateButton.evaluate(el =>
  ...
  test('should allow users to recalculate with different inputs', async ({ page }) => {
  ...
  test('Individual question validation - Q1 Confidence', async ({ page }) => {
    ...
    await selectRadio(page, 'hard enough for penetration', 'Sometimes (about half the time)'); // 3 points
    await selectRadio(page, 'able to maintain', 'Sometimes (about half the time)'); // 3 points
    await selectRadio(page, 'how difficult', 'Difficult'); // 3 points
    await selectRadio(page, 'how often was it satisfactory', 'Sometimes (about half the time)'); // 3 points
  ...
  test('Individual question validation - Q2 Firmness', async ({ page }) => {
    ...
    await selectRadio(page, 'hard enough for penetration', 'Almost never or never'); // 1 point
    await selectRadio(page, 'able to maintain', 'Sometimes (about half the time)'); // 3 points
    await selectRadio(page, 'how difficult', 'Difficult'); // 3 points
    await selectRadio(page, 'how often was it satisfactory', 'Sometimes (about half the time)'); // 3 points
  ...
  test('Individual question validation - Q5 Satisfaction', async ({ page }) => {
    ...
    await selectRadio(page, 'able to maintain', 'Sometimes (about half the time)'); // 3 points
    await selectRadio(page, 'how difficult', 'Difficult'); // 3 points
    await selectRadio(page, 'how often was it satisfactory', 'Almost never or never'); // 1 point
