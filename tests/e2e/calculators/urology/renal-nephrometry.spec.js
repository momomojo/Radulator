...
test.describe('R.E.N.A.L. Nephrometry Score Calculator', () => {
  ...
  test.beforeEach(async ({ page }) => {
    ...
    await expect(page.locator('h2:has-text("RENAL Nephrometry Score")')).toBeVisible();
  });

  test('should display calculator information and description', async ({ page }) => {
    // Verify calculator description
    await expect(page.locator('text=Quantifies anatomical characteristics of renal tumours')).toBeVisible();

    // Verify info box is present
    const infoBox = page.locator('.bg-blue-50\\/60').first();
  ...
  test('should display all input fields', async ({ page }) => {
    ...
    await expect(page.locator('label:has-text("Tumor diameter")')).toBeVisible();
    await expect(page.locator('label:has-text("Exophytic/endophytic nature")')).toBeVisible();
    await expect(page.locator('label:has-text("Nearness to collecting system")')).toBeVisible();
    await expect(page.locator('label:has-text("Location relative to polar line")')).toBeVisible();
    await expect(page.locator('label:has-text("Anterior/posterior location")')).toBeVisible();
    await expect(page.locator('label:has-text("Hilar location")')).toBeVisible();
  });
  ...
  test.describe('Comprehensive Test Cases from Fixtures', () => {
    for (const testCase of testData.testCases) {
      test(`${testCase.id}: ${testCase.description}`, async ({ page }) => {
        ...
        await selectOption(page, 'Anterior/posterior location', testCase.inputs.anterior);
        ...
        // Verify total score
        const totalScoreText = await resultsSection.locator('p:has-text("Total Score")').textContent();
        expect(totalScoreText).toContain(testCase.expected.totalScore);

        // Verify complexity
        const complexityText = await resultsSection.locator('p:has-text("Complexity")').textContent();
        expect(complexityText).toContain(testCase.expected.complexity);

        // Verify interpretation contains expected text
        const interpretationText = await resultsSection.locator('p:has-text("Interpretation")').textContent();
  ...
  test.describe('R Component - Radius/Size Testing', () => {
    test('should calculate R=1 for tumor ≤ 4.0 cm', async ({ page }) => {
      ...
      await selectRadio(page, 'Location relative to polar line', 'Entirely above or below polar line');
      ...
      await expect(results.locator('p:has-text("Total Score")')).toContainText('4a');
      await expect(results.locator('p:has-text("Complexity")')).toContainText('Low complexity');
    });

    test('should calculate R=1 for tumor exactly 4.0 cm (boundary)', async ({ page }) => {
    ...
    test('should calculate R=2 for tumor > 4.0 and < 7.0 cm', async ({ page }) => {
      ...
      const results = page.locator('section[aria-live="polite"]');
      await expect(results.locator('p:has-text("Total Score")')).toContainText('5a');
    });

    test('should calculate R=3 for tumor exactly 7.0 cm (boundary)', async ({ page }) => {
      ...
      const results = page.locator('section[aria-live="polite"]');
      await expect(results.locator('p:has-text("Total Score")')).toContainText('6a');
    });
  ...
  test.describe('E Component - Exophytic/Endophytic Testing', () => {
    ...
    test('should calculate E=3 for entirely endophytic', async ({ page }) => {
      await fillInput(page, 'Tumor diameter', '4.0');
      await selectRadio(page, 'Exophytic/endophytic nature', 'Entirely endophytic');
      await selectRadio(page, 'Nearness to collecting system', '≥ 7 mm');
      await selectRadio(page, 'Location relative to polar line', 'Entirely above or below polar line');
      await selectOption(page, 'Anterior/posterior location', 'Anterior');
  ...
  test.describe('N Component - Nearness to Collecting System Testing', () => {
    test('should calculate N=1 for ≥ 7 mm from collecting system', async ({ page }) => {
      await fillInput(page, 'Tumor diameter', '4.0');
      await selectRadio(page, 'Exophytic/endophytic nature', '≥ 50% exophytic');
      await selectRadio(page, 'Nearness to collecting system', '≥ 7 mm');
      await selectRadio(page, 'Location relative to polar line', 'Entirely above or below polar line');
    ...
    test('should calculate N=2 for 4-7 mm from collecting system', async ({ page }) => {
      await fillInput(page, 'Tumor diameter', '4.0');
      await selectRadio(page, 'Exophytic/endophytic nature', '≥ 50% exophytic');
    ...
    test('should calculate N=3 for ≤ 4 mm from collecting system', async ({ page }) => {
      await fillInput(page, 'Tumor diameter', '4.0');
      await selectRadio(page, 'Exophytic/endophytic nature', '≥ 50% exophytic');
  ...
  test.describe('L Component - Location Relative to Polar Lines Testing', () => {
    test('should calculate L=1 for entirely above or below polar line', async ({ page }) => {
      await fillInput(page, 'Tumor diameter', '4.0');
      await selectRadio(page, 'Exophytic/endophytic nature', '≥ 50% exophytic');
      await selectRadio(page, 'Nearness to collecting system', '≥ 7 mm');
      await selectRadio(page, 'Location relative to polar line', 'Entirely above or below polar line');
    ...
    test('should calculate L=2 for crosses polar line', async ({ page }) => {
      ...
      await selectRadio(page, 'Exophytic/endophytic nature', '≥ 50% exophytic');
      await selectRadio(page, 'Nearness to collecting system', '≥ 7 mm');
      await selectRadio(page, 'Location relative to polar line', 'Crosses polar line');
      await selectOption(page, 'Anterior/posterior location', 'Anterior');
    ...
    test('should calculate L=3 for >50% across polar line or central', async ({ page }) => {
      await fillInput(page, 'Tumor diameter', '4.0');
      await selectRadio(page, 'Exophytic/endophytic nature', '≥ 50% exophytic');
      await selectRadio(page, 'Nearness to collecting system', '≥ 7 mm');
      await selectRadio(page, 'Location relative to polar line', '>50% across polar line/between lines/crosses midline');
      await selectOption(page, 'Anterior/posterior location', 'Anterior');
  ...
  test.describe('A Descriptor - Anterior/Posterior Location Testing', () => {
    ...
    test('should append "p" suffix for Posterior location', async ({ page }) => {
      await fillInput(page, 'Tumor diameter', '4.0');
      await selectRadio(page, 'Exophytic/endophytic nature', '≥ 50% exophytic');
      await selectRadio(page, 'Nearness to collecting system', '≥ 7 mm');
      await selectRadio(page, 'Location relative to polar line', 'Entirely above or below polar line');
      ...
      const results = page.locator('section[aria-live="polite"]');
      await expect(results.locator('p:has-text("Total Score")')).toContainText('4p');
    });

    test('should append "x" suffix for Neither/Indeterminate location', async ({ page }) => {
      await fillInput(page, 'Tumor diameter', '4.0');
      await selectRadio(page, 'Exophytic/endophytic nature', '≥ 50% exophytic');
      await selectRadio(page, 'Nearness to collecting system', '≥ 7 mm');
      await selectRadio(page, 'Location relative to polar line', 'Entirely above or below polar line');
      await selectOption(page, 'Anterior/posterior location', 'Neither/Indeterminate');

      await page.click('button:has-text("Calculate")');
      await page.waitForTimeout(500);

      const results = page.locator('section[aria-live="polite"]');
      await expect(results.locator('p:has-text("Total Score")')).toContainText('4x');
    });
  });

  test.describe('Hilar Suffix Testing', () => {
    test('should append "h" suffix when hilar involvement is checked', async ({ page }) => {
      await fillInput(page, 'Tumor diameter', '4.0');
      await selectRadio(page, 'Exophytic/endophytic nature', '≥ 50% exophytic');
      await selectRadio(page, 'Nearness to collecting system', '≥ 7 mm');
      await selectRadio(page, 'Location relative to polar line', 'Entirely above or below polar line');
      ...
      const results = page.locator('section[aria-live="polite"]');
      await expect(results.locator('p:has-text("Total Score")')).toContainText('4ah');
    });

    test('should not append "h" when hilar involvement is unchecked', async ({ page }) => {
      await fillInput(page, 'Tumor diameter', '4.0');
      await selectRadio(page, 'Exophytic/endophytic nature', '≥ 50% exophytic');
      await selectRadio(page, 'Nearness to collecting system', '≥ 7 mm');
      await selectRadio(page, 'Location relative to polar line', 'Entirely above or below polar line');
      ...
      const results = page.locator('section[aria-live="polite"]');
      const scoreText = await results.locator('p:has-text("Total Score")').textContent();
      expect(scoreText).toContain('4a');
      expect(scoreText).not.toContain('4ah');
    });

    test('should combine all suffixes: posterior + hilar = "ph"', async ({ page }) => {
      await fillInput(page, 'Tumor diameter', '4.0');
      await selectRadio(page, 'Exophytic/endophytic nature', '≥ 50% exophytic');
      await selectRadio(page, 'Nearness to collecting system', '≥ 7 mm');
      await selectRadio(page, 'Location relative to polar line', 'Entirely above or below polar line');
      ...
      const results = page.locator('section[aria-live="polite"]');
      await expect(results.locator('p:has-text("Total Score")')).toContainText('4ph');
    });

    test('should combine all suffixes: indeterminate + hilar = "xh"', async ({ page }) => {
      await fillInput(page, 'Tumor diameter', '4.0');
      await selectRadio(page, 'Exophytic/endophytic nature', '≥ 50% exophytic');
      await selectRadio(page, 'Nearness to collecting system', '≥ 7 mm');
      await selectRadio(page, 'Location relative to polar line', 'Entirely above or below polar line');
      ...
      const results = page.locator('section[aria-live="polite"]');
      await expect(results.locator('p:has-text("Total Score")')).toContainText('4xh');
    });
  });
  ...
  test.describe('Complexity Classification Testing', () => {
    ...
    test('should classify score 7-9 as Moderate complexity', async ({ page }) => {
      ...
      const results = page.locator('section[aria-live="polite"]');
      await expect(results.locator('p:has-text("Total Score")')).toContainText('7a');
      await expect(results.locator('p:has-text("Complexity")')).toContainText('Moderate complexity');
    });
    ...
    test('should classify score 10-12 as High complexity', async ({ page }) => {
      ...
      const results = page.locator('section[aria-live="polite"]');
      await expect(results.locator('p:has-text("Total Score")')).toContainText('10a');
      await expect(results.locator('p:has-text("Complexity")')).toContainText('High complexity');
    });
  });

  test.describe('Boundary Condition Testing', () => {
    test('should classify score 6 as Low (boundary before Moderate)', async ({ page }) => {
    ...
    test('should classify score 7 as Moderate (boundary entering Moderate)', async ({ page }) => {
      await fillInput(page, 'Tumor diameter', '5.5');
      await selectRadio(page, 'Exophytic/endophytic nature', '≥ 50% exophytic');
      await selectRadio(page, 'Nearness to collecting system', '4-7 mm');
      await selectRadio(page, 'Location relative to polar line', 'Crosses polar line');
    ...
    test('should classify score 9 as Moderate (boundary before High)', async ({ page }) => {
      ...
      const results = page.locator('section[aria-live="polite"]');
      await expect(results.locator('p:has-text("Total Score")')).toContainText('9a');
      await expect(results.locator('p:has-text("Complexity")')).toContainText('Moderate complexity');
    });

    test('should classify score 10 as High (boundary entering High)', async ({ page }) => {
  ...
  test.describe('Clinical Interpretation Testing', () => {
    test('should provide appropriate interpretation for Low complexity', async ({ page }) => {
      await fillInput(page, 'Tumor diameter', '3.5');
      await selectRadio(page, 'Exophytic/endophytic nature', '≥ 50% exophytic');
      await selectRadio(page, 'Nearness to collecting system', '≥ 7 mm');
      await selectRadio(page, 'Location relative to polar line', 'Entirely above or below polar line');
      ...
      const results = page.locator('section[aria-live="polite"]');
      const interpretation = await results.locator('p:has-text("Interpretation")').textContent();
    ...
    test('should provide appropriate interpretation for Moderate complexity', async ({ page }) => {
      await fillInput(page, 'Tumor diameter', '5.5');
      await selectRadio(page, 'Exophytic/endophytic nature', '≥ 50% exophytic');
      await selectRadio(page, 'Nearness to collecting system', '4-7 mm');
      await selectRadio(page, 'Location relative to polar line', 'Crosses polar line');
    ...
    test('should provide appropriate interpretation for High complexity', async ({ page }) => {
      await fillInput(page, 'Tumor diameter', '9.0');
      await selectRadio(page, 'Exophytic/endophytic nature', 'Entirely endophytic');
      await selectRadio(page, 'Nearness to collecting system', '≤ 4 mm');
      await selectRadio(page, 'Location relative to polar line', '>50% across polar line/between lines/crosses midline');
  ...
  test.describe('Reference Links Testing', () => {
    test('should display all reference links', async ({ page }) => {
      ...
      await expect(referencesSection.locator('a:has-text("Kutikov")')).toBeVisible();
      await expect(referencesSection.locator('a:has-text("Canter")')).toBeVisible();
      await expect(referencesSection.locator('a:has-text("Rosevear")')).toBeVisible();
      await expect(referencesSection.locator('a:has-text("AUA Guidelines")')).toBeVisible();
    });

    test('should have valid reference links', async ({ page }) => {
      const links = [
        { text: 'Kutikov', url: 'https://doi.org/10.1016/j.juro.2009.05.035' },
        { text: 'Canter', url: 'https://doi.org/10.1016/j.urology.2011.04.035' },
  ...
  test.describe('Visual and Theme Consistency', () => {
    ...
    test('should display diagram link', async ({ page }) => {
      const diagramLink = page.locator('button:has-text("View RENAL Score Diagram")');
