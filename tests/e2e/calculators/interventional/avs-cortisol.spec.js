...
test.describe("AVS Cortisol Calculator", () => {
  test.beforeEach(async ({ page }) => {
    ...
    await expect(
      page.locator('h3:has-text("Patient Information")'),
    ).toBeVisible();
  });

  test("should load calculator with all required sections", async ({
    page,
  }) => {
    ...
    await expect(
      page.locator('h3:has-text("Peripheral (IVC) Measurements")'),
    ).toBeVisible();
    await expect(
      page.locator('h3:has-text("Left Adrenal Vein Samples")'),
    ).toBeVisible();
    await expect(
      page.locator('h3:has-text("Right Adrenal Vein Samples")'),
    ).toBeVisible();
  ...
  test("should have correct patient metadata fields", async ({ page }) => {
    ...
    const sideSelect = page
      .locator('label:has-text("Side of Nodule") ~ select')
      .first();
  ...
  test("should have unit selection for cortisol", async ({ page }) => {
    const cortUnitsSelect = page
      .locator('label:has-text("Cortisol Units") ~ select')
      .first();
  ...
  test("should support multiple left adrenal vein samples (up to 2)", async ({
    ...
    const initialSamples = await page
      .locator('label:has-text("Time Drawn")')
      .count();
    ...
    const samplesAfterAdd = await page
      .locator('label:has-text("Time Drawn")')
      .count();
  ...
  test("should support multiple right adrenal vein samples (up to 4)", async ({
    page,
  }) => {
    const addRightButton = page.locator(
      'button:has-text("+ Add Right Sample")',
    );
  ...
  test("should calculate unilateral left adrenal adenoma (Test Case 1)", async ({
    page,
  }) => {
    // Patient metadata
    ...
    await page
      .locator('label:has-text("Infrarenal IVC Cortisol") ~ input')
      .first()
      .fill("15");
    await page
      .locator('label:has-text("Infrarenal IVC Epinephrine") ~ input')
      .first()
      .fill("50");
    ...
    const interpretation = page.locator(
      "text=Unilateral cortisol-secreting adenoma on LEFT side",
    );
    await expect(interpretation).toBeVisible();
  });

  test("should calculate unilateral right adrenal adenoma (Test Case 2)", async ({
    page,
  }) => {
    // Patient metadata
    ...
    // Verify interpretation
    const interpretation = page.locator(
      "text=Unilateral cortisol-secreting adenoma on RIGHT side",
    );
    await expect(interpretation).toBeVisible();
  ...
  test("should detect bilateral cortisol hypersecretion (Test Case 3)", async ({
    page,
  }) => {
    // Patient metadata
    ...
    const interpretation = await page
      .locator('h3:has-text("Interpretation")')
      .locator("..")
      .locator("p")
      .first()
      .textContent();
  ...
  test("should detect failed cannulation when epinephrine delta <100 (Test Case 4)", async ({
    page,
  }) => {
    // Patient metadata
  ...
  test("should support multi-sample averaging for left adrenal vein", async ({
    page,
  }) => {
  ...
  test("should support cortisol unit conversion (nmol/L to µg/dL)", async ({
    page,
  }) => {
  ...
  test("should prefer suprarenal IVC over infrarenal when both provided", async ({
    page,
  }) => {
    ...
    await page
      .locator('label:has-text("Suprarenal IVC Cortisol") ~ input')
      .first()
      .fill("15");
    await page
      .locator('label:has-text("Suprarenal IVC Epinephrine") ~ input')
      .first()
      .fill("50");
  ...
  test("should display CSV download button after calculation", async ({
    page,
  }) => {
    // Fill minimal data
    await page
  ...
  test("should show error when insufficient data provided", async ({
    page,
  }) => {
  ...
  test("should have consistent theme styling", async ({ page }) => {
    ...
    const hasRoundedBorders = await patientInfoSection.evaluate(
      (el) =>
  ...
  test("should display interpretation based on Young et al. criteria", async ({
    page,
  }) => {
  ...
  test("should handle edge case: very high cortisol levels", async ({
    page,
  }) => {
  ...
  test("should handle edge case: minimal epinephrine gradient (exactly 100)", async ({
    page,
  }) => {
    await page
      .locator('label:has-text("Infrarenal IVC Cortisol") ~ input')
