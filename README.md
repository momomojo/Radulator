...
Radulator is a modern web application providing evidence-based medical calculators for radiology and adjacent specialties. It features **41 specialized calculators across 11 specialty categories**, each with evidence-based formulas, proper academic references, and built-in result interpretation. Calculators are auto-discovered from their own metadata (`src/components/calculators/registry.js`), so this list always reflects the code:
...
ACR BI-RADS (Breast Imaging) · ACR O-RADS (Women's Imaging) · CAC/MESA Calculator (Cardiac Imaging) · CAD-RADS 2.0 (Cardiac Imaging) · Cockcroft-Gault eCrCl (Nephrology) · Mehran CIN Risk Score (Nephrology) · AAST Trauma Grading (Trauma)
...
├── src/
│   ├── components/
│   │   ├── calculators/       # 41 calculator components (one .jsx each)
