export const HipDysplasiaIndices = {
  ...
  tags: ["Radiology", "Pediatric", "Orthopedic"],
  metaDesc:
  ...
  compute: (v) => {
    ...
    if (mi_right_a && mi_right_b) {
      const miRight =
        (parseFloat(mi_right_a) /
          (parseFloat(mi_right_a) + parseFloat(mi_right_b))) *
        100;
      result["Migration Index (Right)"] =
        `${miRight.toFixed(1)}% - ${interpretMigrationIndex(miRight, ageMonths)}`;
    }

    if (mi_left_a && mi_left_b) {
      const miLeft =
        (parseFloat(mi_left_a) /
          (parseFloat(mi_left_a) + parseFloat(mi_left_b))) *
        100;
      result["Migration Index (Left)"] =
        `${miLeft.toFixed(1)}% - ${interpretMigrationIndex(miLeft, ageMonths)}`;
    }
