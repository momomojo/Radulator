export const SpleenSizeULN = {
  ...
  compute: ({ gender, height = 0 }) => {
    ...
    const validRange = gender === "female" 
      ? (h >= 155 && h <= 180)
      : (h >= 165 && h <= 200);
    
    const result = {
      "Spleen length should not exceed (cm)": spleenLength.toFixed(1),
      "Spleen volume should not exceed (cm³)": Math.round(spleenVolume).toString(),
    };
    
    if (validRange) {
      result["Interpretation"] = "The above mentioned spleen size and volume are within 95% of observations in healthy individuals.";
    } else {
      const range = gender === "female" ? "155-180 cm" : "165-200 cm";
      result["Warning"] = `Height outside validated range (${range}). Results may be less accurate.`;
      result["Interpretation"] = "The above mentioned spleen size and volume are within 95% of observations in healthy individuals (extrapolated beyond validated range).";
    }
