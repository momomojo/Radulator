...
function renderAudit(data) {
  return `${JSON.stringify(
    {
      schema: "radulator-mesa-cac-reference-audit/v1",
      auditedAt: new Date().toISOString(),
      officialCalculator: OFFICIAL_MESA_CALCULATOR,
      tlsVerified: true,
      groupCount: buildReferenceGroups().length,
      dataSha256: hashReferenceData(data),
      groups: Object.entries(data).map(([key, record]) => ({
        key,
        p: record.p,
        r: record.r,
...
function parseCli(argv) {
  ...
  if (!options.output || !options.auditOutput) {
    throw new Error("Usage: --output FILE --audit-output FILE [--concurrency 1-8]");
  }
  return options;
}
...
async function main() {
  ...
  await writeFile(options.auditOutput, renderAudit(data), { mode: 0o644 });
  console.log(
    JSON.stringify({
      ok: true,
